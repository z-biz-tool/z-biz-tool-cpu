  ...
  return (
    <div className="panel-body help">
      ...
      <div className="panel-note">
        ...
        焦点在可访问视图的元件清单、连接表或波形清单里时同样不生效：那三块用方向键走位，空格是「选中这个元件」而不是「运行」。
      </div>
      ...
      <table className="help-keys">
        <tbody>
          {[
            ...
            ["↑ ↓ ← → / Home / End", "在可访问视图的元件清单或连接表里走访：每张表只占一个 Tab 停靠点，走到的那一个会念出名称、类型与引脚数（连接表念的是这根线的两端）"],
            ["回车 / 空格", "把元件清单里当前这一行的元件选到画布上（画布与属性面板跟着同步）；焦点落在别的按钮上时就是按下那个按钮"],
            ["未连接端口清单里按回车", "把这一行的端口选作连线起点（画布上的橡皮筋跟着走），再在另一行按回车就落一根线；Esc 或「取消连线」按钮放弃"],
            ["选中连接表里的一根线后", "表下方的「断开这根线」只拆不接；「从 X 改接」先把这根线拆掉、把 X 那一端留在手上，再去未连接端口清单点新端点就接过去了（一次撤销回到改接之前）"],
          ].map(([k, v]) => (
            <tr key={k}>
              <td>
                <code>{k}</code>
      ...
      <div className="panel-note">
        ...
        「未连接端口」那一列同样是按钮：按两次（起点、终点）就在键盘上完成一根连线，不必去点画布上几像素大的引脚。
